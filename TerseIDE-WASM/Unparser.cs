                ...
                case FunctionInvocationExpression funcExpr:
                    ...
                    if (Function.IsHigherOrder(funcExpr.Function, caller.ObjectType)) {
                        ...
                        if (funcExpr.Arguments[0] is AutoExpression) {
                            ...
                            (lambda, parameterList) = CreateLambda(funcExpr.Arguments[0], func.LambdaParameters);
                            createdLambda = true;
                        }
                        ...
                        if (createdLambda)
                            unparsedFunc = $"{unparsedCaller}.{funcExpr.Function}({paramDeclare}=>{Unparse(funcExpr.Arguments[0]).unparsed})";
                    ...
                    if (Function.IsUnary(funcExpr.Function)) {
                        ...
                        VObject arg = funcExpr.Arguments[0] is AutoExpression ? programState.Autofill_2 : Unparse(funcExpr.Arguments[0]).val;
                        string unparsedArg = funcExpr.Arguments[0] is AutoExpression ? programState.Autofill2Name : Unparse(funcExpr.Arguments[0]).unparsed;
